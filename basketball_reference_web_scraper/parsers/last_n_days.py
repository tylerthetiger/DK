...
def parse_last_n_days_playerlist_row(row):
    #print type(row)
  #  print type(row[0])
   # print row[0]
    for a in row:
        playerName= a.text_content()
        htmlLink= a.get("href")
        return (playerName,htmlLink)
    return None
#Rk	Player	Tm	G	GS	MP	FG	FGA	FG%	3P	3PA	3P%	FT	FTA	FT%	ORB	DRB	TRB	AST	STL	BLK	TOV	PF	PTS	GmSc
    ...
    # }
def parse_last_n_days_playerlist(page):
    tree = html.fromstring(page)
    rows = tree.cssselect("[class='left ']")
    #rows = tree.xpath('//table[@id="players"]//tbody/tr[not(contains(@class, "thead"))]')
    #rows = tree.xpath('//table[@id="players"]//tbody//tr//td[@class="left"]')
    #rows = tree.xpath('//td[@id="left"]')
    playerList = []
    for row in rows:
        lastnRet = parse_last_n_days_playerlist_row(row)
        if lastnRet != None:
            playerList.append(lastnRet)
    return playerList
